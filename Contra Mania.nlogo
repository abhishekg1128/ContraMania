breed [ zombies zombie ]                              ; zombies are the bad man want to kill us
breed [ cannons cannon ]                              ; we will fire on zombies by cannon this will be only one
breed [ missiles missile ]                            ; missiles are the bulltes fired by cannon
breed [ s_r_ps s_r_p ]                                ; s_r_p is a s_r_pup which after hitting by missile repaires our shild and fires many missiles in different directions
breed [ h_100_ps h100_p ]                             ; This is a powerup to increaser the Health by 50

globals [ z bullets score reload-time head ]

; z is the number of zombies
; bullets is the number of missiles loaded in cannon
; score is the number of zombies killed
; reload-time is the time after which we can reload our cannon again
; head is a variable which is userd to set heading of missiles after taking a s_r_pup
; Health is the Health of our cannon

to setup                                                               ; this function sets up the game

  clear-all
  reset-ticks

  set z 0                                                              ; resetting all variables
  set score 0
  set Health 100
  set bullets 10
  set reload-time 10

  set-default-shape zombies "person"                                   ; Setting the required shape for all breeds
  set-default-shape missiles "dart"
  set-default-shape s_r_ps "fire"
  set-default-shape h_100_ps "strawberry"

  create-cannons 1                                                     ; Creating the cannon
  [
    set shape "cannon"
    setxy 0 0
    set heading 0
  ...
  ask patches[                                                         ; Setting the background
    set pcolor 65
  ]

  shield                                                               ; Calling function shield to build a shield
end

to shield                                                              ; function to build shield
  ask patches[
    if( pxcor = 3 or pxcor = -3 )
    [
      if( pycor > -4 and pycor < 4 )
      [ set pcolor 60 ]
    ]

    if( pycor = 3 or pycor = -3 )
    [
...
to play                                                                ; Function to start the game
  tick-advance 1

  if( ticks mod 15 = 0 )                                               ; Whenever ticks reaches the multiple of 15 a new zombie will be created
  [
    set z z + 1
...
; Next block is used to kill zombies, to activate s_r_pups if missile hit them and to move missiles forward

  ask missiles [
    if any? zombies-here [
      set score score + 1

      set z z - count zombies-here
    ...
    if any? s_r_ps-here [                                              ; If any missile hits a s_r_pup then head will be 1
      set head 1
    ]

    if any? h_100_ps-here [
      ifelse health < 51
    ...
    if ycor >= 15 or ycor <= -15 or xcor >= 15 or ycor <= -15          ; If missiles reaches to the end of arena then they will die
    [ die ]
  ]

  if head = 1 [  s_r_p-up  ]                                           ; If head will be 1 then s_r_p-up function is called to activate the the s_r_pup

; Next block is used to check if any zombie is on our shield, if it is then they start breaking it else they move forward

  ask zombies
  [
     ifelse pcolor < 65
     [  if ticks mod 8 = 0 [ set pcolor pcolor + 1 ]  ]
     [
       ifelse xcor >= -1 and xcor <= 1 and ycor >= -1 and ycor <= 1      ; If any zombie gets close to our cannon then we die
       [   if ticks mod 2 = 0 [ set Health Health - 1 ] ]
       [  forward 0.2  ]
     ]

  ]

  if Health < 1                                                          ; If health will be smaller than 1 game stops
  [
    user-message (word "Game Over!!! Your score is " score "." )
    stop
  ]

  if ticks mod 15 = 0 and reload-time > 0                              ; To update the reload time
  [  set reload-time reload-time - 1  ]

  if random(300) = 125 [                                                ; To create shileld repair powerups
    create-s_r_ps 1 [
  ...
  if random(800) = 400 [                                               ; To create health restore powerup
    create-h_100_ps 1 [
      setxy 15 - random(30) 15 - random(30)
      set color red
    ]
...
to clock                                                               ; To rotate connon in clockwise direction
  ask cannons
  [
...
to anti-clock                                                          ; To rotate connon in anti-clockwise direction
  ask cannons
  [
     set heading heading + 10
  ]
...
to fire                                                                ; To fire missiles

  if bullets < 1 [ stop ]

  create-missiles 1[
    set heading [heading] of cannon 0
...
to reload                                                               ; To reload missiles
  if reload-time > 0 [ stop ]
  set bullets 10
  set reload-time 10
end
...
1273
451
Welcome to Contra Mania.\n\nCID confirmed that today many enemies will be coming for a surgical strike in INDIA, hence we Indians have built a bunker to hide ourselfs while Indian army will be protecting us.\n\nAs a commander you have to lead a cannon to attack on enemies to protect your country men.\n\nYou will be provided upgrades to restore your shield and health randomly.\n\nIf you lose your country lose.\n\nBest of Luck .......
20
95.0
0
